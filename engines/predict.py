...
class Predictor:
    ...
    def predict_one(self, sentence):
        ...
        if self.configs.use_pretrained_model:
            X, y, att_mask, Sentence = self.dataManager.prepare_single_sentence(sentence)  # 此时的y是用全'O'来填补的
        ...
        if self.configs.use_pretrained_model:
            # 去掉[CLS]和[SEP]对应的位置
            y_pred = y_pred[1:-1]
        entities, suffixes, indices = extract_entity(sentence, y_pred, self.dataManager)  # 完成从label的id到输出实体名、标签名和位置的转换
