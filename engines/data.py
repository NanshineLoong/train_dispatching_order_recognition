...
class DataManager:
    ...
    def __init__(self, configs, logger):
        ...
        # 载入词表文件，如果没有词表文件，则用训练集的token和label分别生成相应词表文件，再载入（注意：如果使用了预训练模型，则不会生成或载入token的词表文件）

        if configs.use_pretrained_model:  # 如果使用预训练模型，则载入tokenizer
            if configs.pretrained_model == 'Bert':
                from transformers import BertTokenizer
                self.tokenizer = BertTokenizer.from_pretrained('bert-base-chinese')
            self.max_token_number = len(self.tokenizer)  # 词表长度
        else:
        ...
        with tqdm(total=lines, desc='loading data') as bar:
            for index, record in df.iterrows():
                ...
                if str(token) == str(np.nan):
                    if len(tmp_x) <= self.max_sequence_length - 2:  # 为什么-2？
                        if tmp_x == []:
                            import pdb  
                            pdb.set_trace()
                            # print(tmp_x + ' chucuole')
    ...
    def get_training_set(self, train_val_ratio=0.9):
        ...
        if self.configs.use_pretrained_model:
            X, y, att_mask = self.prepare_pretrained_embedding(df_train)  # bert对输入的token编码，用label的vocab对标签编码
            # shuffle the samples
            num_samples = len(X)
