...
class Train:
    def __init__(self, configs, data_manager, logger):
        ...
        if configs.use_pretrained_model and not configs.finetune:  # 使用预训练模型但不微调：将预训练模型载入self.pretrained_model中，将不会对预训练模型做参数调整？
            if configs.pretrained_model == 'Bert':
                from transformers import TFBertModel
                self.pretrained_model = TFBertModel.from_pretrained('bert-base-chinese')

        self.ner_model = NerModel(configs, vocab_size, num_classes)   # 使用预训练模型并微调：将预训练模型载入self.ner_model中，将会对预训练模型做参数调整？

        checkpoint = tf.train.Checkpoint(ner_model=self.ner_model)  # 通过checkpoints_dir和checkpoint_name载入之前训练的模型参数，来做叠加训练？
        self.checkpoint_manager = tf.train.CheckpointManager(   
            checkpoint, directory=checkpoints_dir, checkpoint_name=checkpoint_name, max_to_keep=max_to_keep)  # max_to_keep是最大的模型保存数量？
        checkpoint.restore(self.checkpoint_manager.latest_checkpoint)  # 这里导入模型。最新的模型？
        if self.checkpoint_manager.latest_checkpoint:  # 有模型则声明成功导入
            print('Restored from {}'.format(self.checkpoint_manager.latest_checkpoint))
        else:
            print('Initializing from scratch.') # 无存储的模型，从头初始化
        ...
        for i in range(self.epoch):
            ...
            for step, batch in tqdm(train_dataset.shuffle(len(train_dataset)).batch(self.batch_size).enumerate()):
                ...
                if step % self.configs.print_per_batch == 0 and step != 0:
                    batch_pred_sequence, _ = crf_decode(logits, transition_params, inputs_length)  # batch_pred_sequence 就是对这个batch里y的预测值（每一个label的预测索引）
                    measures, _ = metrics(
            ...
            if np.array(val_f1_avg).mean() > best_f1_val:
                ...
                best_at_epoch = i + 1
                self.checkpoint_manager.save()  # 保存结果更好的结果，（只保存ner_model的参数，对于没有微调的预训练模型不会保存)
